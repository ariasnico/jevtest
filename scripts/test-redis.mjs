...
await assert.rejects(store.talk(id,failed,{...deps,evaluate:async()=>{throw Error('provider unavailable');}}),{code:'evaluation_unavailable'});
...
await store.start(recoveryId);
const recoveryRequest={turnId:randomUUID(),expectedVersion:0,message:'A concrete plan'};
const recovered=await store.talk(recoveryId,recoveryRequest,{...deps,generate:async()=>{throw Error('writer outage');}});
assert.equal(recovered.turns,1);assert.equal(recovered.score,42);
assert.deepEqual(await store.talk(recoveryId,recoveryRequest,deps),recovered);
assert.equal((await store.resume(recoveryId)).turns,1);
