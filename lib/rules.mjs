...
export function decideTurn(game,message,evaluation) {
  ...
  const mood=status==='won'?(vip?'Uno más de la mesa':'Estás adentro'):status==='lost'?(vip?'El VIP queda para otra noche':'Hoy no se pudo'):vip?(delta<0?'Le bajaste la onda':score>=60?'Hay código':score>=35?'Rompiste el hielo':'Te está midiendo'):delta>0?'Está aflojando':delta<0?'No compra el chamuyo':'Te está escuchando';
